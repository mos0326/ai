...
前提: `python3` と `node`(npm) が入っていること。
...
Windows はエクスプローラで **`start.bat` をダブルクリック**（またはコマンドプロンプトで `start.bat`）。

初回は依存を自動でインストールし（数分かかることがあります）、バックエンド(8000)とフロント(5173)を
同時に起動して、数秒後にブラウザで **http://localhost:5173** を自動で開きます。停止は `Ctrl+C`
（Windows は開いた2つのウィンドウを閉じる）。

> 開いたターミナル/ウィンドウは動いたままが正常です（サーバーが常駐しています）。
> 自動で開かない場合は手動で http://localhost:5173 を開いてください。

> APIキーが無くても起動します（チャット等は無効表示）。全機能を使うには
> `backend/.env`（初回起動時に自動生成）に `ANTHROPIC_API_KEY` などを記入して、もう一度 `./start.sh`。
